...
> **Where to start, as at 2026-08-26.** Agent-actionable now, in order: **12** (NRPG directory),
> 14, 17, 19, 21, 23, 24, **26** (newly unblocked by #10), 27. Rows 10 and 11 are done; 11's
> email-capture half is founder-gated (DECISIONS #21). Every P0 row is done, closed, or
> founder-only.
>
> **Before building anything, read the "queued push" section in Discoveries.** Four branches from
> this session sit unpushed with a fixed merge order, and the omnibus branch
> `archive/carsi-omnibus-260826-superseded` must **not** be merged — it is a superset that would
> undo the split.
>
> **Audit before you build.** Rows 10 and 11 both read `ready` and both were already built; row 11's
> reminder funnel was running daily in production. `SPEC.md` §2 carries the four-command sweep that
> catches this.
...
| 2 | Generate CEC submission packs for top 9 courses (`generate-cec-submission.ts`) + cover email draft | A→F | 0 | **agent half done 2026-08-26** — 9 packs + cover email draft in `docs/cec-submissions/`. A tenth, `wrt-water-damage-essentials`, is HELD BACK in `docs/cec-submissions/held-back/`: its slug brands a CARSI course with an IICRC discipline acronym, so it must not be sent until the slug is renamed and a 301 is in place. Selected by educational hours among the 24 published courses, restricted to courses that map to an IICRC discipline. `ccw-carsi-truckmount-operations` (6 h) and `floor-care-onboarding-operational-readiness` (8 h) are the two longest published courses and were EXCLUDED on purpose: they are not restoration courses, and CLAUDE.md names the truckmount one as the exact incident where IICRC framing was templated onto a course with no IICRC discipline. Sending is DECISIONS #1, founder-only. **Provider name SETTLED 2026-08-26:** the founder confirmed "Centre for Australian Restoration and Standards Information". The wrong name is gone from the repo — corrected in 6 places (the generator literal, three spots on the avian-influenza page including its structured-data `alternateName`, the contact page title, and the workshop media manifest), and the packs were REGENERATED rather than find-and-replaced. **Directory-listing claim CUT 2026-08-26** on founder decision. Every pack used to assert CARSI is "listed in the IICRC CEC Provider Directory"; it could NOT be verified (`iicrccecevents.com` resolves to 97.74.190.62 but returns no HTTP response here, while `iicrc.org` returned 200 in the same run as a positive control), so it was removed from the generator in BOTH places it appeared — the Standing line and a supporting-documents bullet — and the packs regenerated. Recorded as unverified, never as false; restore it by hand if and when the listing is confirmed. **One blocker remains on the packs:** the held-back WRT slug above. The independent review of 364b996e also found the generator deriving a CEC quantity from duration; that inference was removed and all packs regenerated from the corrected source at de149535+. Sending is still DECISIONS #1, founder-only. |
| 3 | Send CEC packs to CECCourse@iicrcnet.org | F | 0 | blocked on #2 |
| 4 | Subscriptions go-live: Stripe Prices + Test Clock + DO env; flip at go-live script exit 0 | A→F | 0 | **DECIDED, UNEXECUTED — founder said "switch the subscriptions on" 2026-08-26 (DECISIONS #2).** The decision is made; only the three manual steps remain, and they are founder-only because performing them means handling a live payment credential, not merely because of policy. **Sequence them after DECISIONS #17** — the `sk_live_` key is currently plaintext in the app spec, and turning a revenue switch on in that environment is the wrong order. The runbook `docs/runbooks/rana-stripe-connection.md` was re-verified against the code on 2026-08-26 and is accurate: price resolution really is `STRIPE_PRICE_PRO_ANNUAL` first then `lookup_key: carsi_pro_annual`, both paths unit-tested, and A$795 matches `pricing-tiers.ts` at `priceCents: 79500`. **agent half green 2026-08-26** — `npm run verify:go-live-readiness` **exit=0**, all automated pre-flight checks pass against the live site: pricing page HTTP 200; subscription-status API fails closed (`has_subscription:false, reason:"none"`); checkout fails closed without a session (HTTP 401, not 200); professional-directory health `stubBlocked:true`. **Read the row title carefully — exit 0 does NOT mean "flip it".** The script's own closing line is "Manual steps remain: create Stripe Prices, run Test Clock checklist, set DO env vars." All three are founder-only: Stripe Prices is money configuration, DO env is production configuration, and the flip itself is DECISIONS #2. Exit 0 means the code side is ready and fails closed correctly, nothing more. |
...
| 13 | Ship authored Google Ads + LinkedIn campaigns at small budget | A→F | 1 | needs budget (DECISIONS #8) — **and do not ship the copy from `main`.** Both campaign documents carried the banned "IICRC CEC-approved" form until 2026-08-26; `git show origin/main:docs/marketing/google-ads-campaign.md \| grep -c "CEC-approved"` still finds it. The corrections live on `docs/carsi-session-260826` and are unmerged, so shipping ads off `main` today would put licence-breaking copy in front of buyers. Merge that branch first. |
...
| 26 | Reference PDFs into employer proof-pack | A | 2 | **unblocked 2026-08-26** — #10 landed, so this is now agent-actionable. **Read `src/lib/server/proof-pack-pdf.ts` and its tests before touching it:** it carries a Unicode fallback (`fontFor` + a subset Noto face) that is doing real work — without it, pdf-lib `StandardFonts` throw on any codepoint outside Latin-1 and a learner with a Māori macron or a Cyrillic name gets a 500 instead of their transcript. Any new text you draw must go through `fontFor`. The font ships via `deploy/Dockerfile:44` (the file `app.yaml` names — the root `Dockerfile` is unused by the deploy) and carsi.com.au serves it at 2,049,096 bytes. Also keep CEC fail-closed: an unapproved course contributes 0. |
