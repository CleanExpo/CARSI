...
function ArticleCard({ article }: { article: ArticleSummary }) {
  return (
    ...
    >
      ...
      <h2
        className="text-lg leading-snug font-semibold text-slate-950 transition-colors group-hover:text-[#146fc2]"
      >
        {article.title}
      </h2>
      {article.excerpt && (
        <p className="line-clamp-3 text-sm leading-relaxed text-slate-500">{article.excerpt}</p>
      )}
  ...
  return (
    <>
      ...
      <CommunityHubShell
        ...
        stats={[
          { value: total > 0 ? String(total) : '—', label: 'Articles' },
          { value: 'AU', label: 'Sourced' },
          { value: 'NRPG', label: 'Practitioner voice' },
