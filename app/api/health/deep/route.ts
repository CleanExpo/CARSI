...
async function checkDatabase(): Promise<DependencyCheck> {
  ...
  if (!upstream) {
    ...
    result.error = 'No external API configured (/ready check skipped)';
    return result;
  }

  try {
    const response = await fetch(`${upstream.replace(/\/$/, '')}/ready`, {
      method: 'GET',
    ...
    if (response.ok) {
      ...
      result.error = `Upstream ready check returned status ${response.status}`;
    }
  } catch (e) {
    result.latency_ms = Date.now() - start;
...
async function checkBackend(): Promise<DependencyCheck> {
  ...
  try {
    const url = upstream
      ? `${upstream.replace(/\/$/, '')}/api/v1/health`
      : `${getBackendOrigin()}${getHealthCheckPath()}`;
