...
| 22 | **Is Stripe reaching the site again?** The webhook endpoint `carsi.com.au/api/lms/webhooks/stripe` was re-registered in Stripe on 29 Aug and is enabled, but both payments since (30 Aug) still show one pending delivery in Stripe and `stripe_webhook_events` holds zero rows; the route answers an unsigned probe with HTTP 400 "Invalid signature", so it is alive. Likely a signing-secret mismatch, but the app config changed on 2 Sep after the last payment, so it may already be fixed. While it fails, refunds, disputes and subscription events never reach the platform and fulfilment rests on the buyer reaching the success page. | Default: **the founder's WS1 test purchase is the test.** If one row appears in `stripe_webhook_events` afterwards, close this. If not, compare the endpoint's signing secret in the Stripe dashboard with `STRIPE_WEBHOOK_SECRET` in the DigitalOcean app and re-copy it — a credential change, founder-only under #17. Nothing is rotated on the hypothesis. | 2026-09-06 | OPEN |
| 23 | **Voice assistant: on or off?** `NEXT_PUBLIC_ELEVENLABS_AGENT_ID` is set in production, so every page tries to load the ElevenLabs widget script from unpkg and the Content-Security-Policy blocks it; the widget never appears and every page logs an error. Allowing a third-party script host is a policy widening, and whether CARSI wants a second (voice) assistant beside Margot is a product call, not a bug fix. | Default: **off** — unset the agent id in the DigitalOcean app if no decision by the deadline; the page is unchanged for visitors either way. | 2026-09-10 | **DECIDED 03/09/2026: off** (Phill, 13:13 AEST on GP-550, relayed by Margot). The widget mount and component were removed (fix 9); revisit when the Margot Knowledge Bot milestone reaches its voice stage. |
