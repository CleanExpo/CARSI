...
function AdminCourseListThumb({
  ...
  // re-attempts rather than staying stuck on the previous failure. Adjusting the
  // reset during render (React's documented "store previous value" pattern) is
...
export function AdminCoursesList() {
  ...
  // Clear the bulk selection whenever the active filter set changes. Adjusting
  // during render (React's documented "store previous value" pattern) is
  // behaviour-identical to a reset-in-effect but avoids the extra commit pass.
  const filterKey = `${status} ${qFromUrl} ${sort} ${cec}`;
