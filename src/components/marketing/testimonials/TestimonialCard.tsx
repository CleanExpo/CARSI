  ...
  return (
    <div className={`flex gap-0.5 ${className}`} aria-label="5 out of 5 stars">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} className="h-3.5 w-3.5 text-[#ed9d24]" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
  ...
  return (
    ...
    >
      ...
      <figcaption className="mt-5 border-t border-slate-200/80 pt-4 dark:border-white/[0.08]">
        ...
        <p className="mt-0.5 text-xs text-slate-500 dark:text-white/45">{testimonial.company}</p>
      </figcaption>
    </figure>
  );
}
