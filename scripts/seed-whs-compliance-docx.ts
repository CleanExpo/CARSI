/**
 * Seed WHS / Compliance courses from `data/CARSI_WHS_Compliance_Courses.docx`.
 * Lesson bodies are stored as plain text (no HTML tags).
 *
 *   DATABASE_URL="..." npx tsx scripts/seed-whs-compliance-docx.ts
 *   DATABASE_URL="..." npx tsx scripts/seed-whs-compliance-docx.ts --replace
 *   npx tsx scripts/seed-whs-compliance-docx.ts --dry-run
 */
...
import { parseWhsComplianceCompendium } from '@/lib/seed/whs-compliance-docx';
import { prisma } from '@/lib/prisma';
import { DEFAULT_INSTRUCTOR_ID, ensureCatalogInstructor } from '@/lib/server/course-catalog-sync';

const __dirname = dirname(fileURLToPath(import.meta.url));
const DOCX_PATH = join(__dirname, '..', 'data', 'CARSI_WHS_Compliance_Courses.docx');
...
async function main() {
  ...
  const courses = parseWhsComplianceCompendium(paras);
