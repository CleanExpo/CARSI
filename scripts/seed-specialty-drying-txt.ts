/**
 * Seed Contents & Specialty Drying courses from `data/contents_specialty_drying_courses.txt`.
 * Lesson bodies are plain text (no HTML).
 *
 *   DATABASE_URL="..." npx tsx scripts/seed-specialty-drying-txt.ts
 *   DATABASE_URL="..." npx tsx scripts/seed-specialty-drying-txt.ts --replace
 *   npx tsx scripts/seed-specialty-drying-txt.ts --dry-run
 */
...
import { parseSpecialtyDryingTxt } from '@/lib/seed/specialty-drying-txt';
import { prisma } from '@/lib/prisma';
import { DEFAULT_INSTRUCTOR_ID, ensureCatalogInstructor } from '@/lib/server/course-catalog-sync';

const __dirname = dirname(fileURLToPath(import.meta.url));
const TXT_PATH = join(__dirname, '..', 'data', 'contents_specialty_drying_courses.txt');
...
async function main() {
  ...
  const courses = parseSpecialtyDryingTxt(raw);

  console.log(`Parsed contents_specialty_drying_courses.txt → ${courses.length} courses.`);
