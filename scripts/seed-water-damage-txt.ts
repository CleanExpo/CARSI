/**
 * Seed Water Damage Restoration courses from `data/water_damage_restoration_courses.txt`.
 * Lesson bodies are plain text (no HTML).
 *
 *   DATABASE_URL="..." npx tsx scripts/seed-water-damage-txt.ts
 *   DATABASE_URL="..." npx tsx scripts/seed-water-damage-txt.ts --replace
 *   npx tsx scripts/seed-water-damage-txt.ts --dry-run
 */
...
const TXT_PATH = join(__dirname, '..', 'data', 'water_damage_restoration_courses.txt');
...
async function main() {
  ...
  console.log(`Parsed water_damage_restoration_courses.txt → ${courses.length} courses.`);
