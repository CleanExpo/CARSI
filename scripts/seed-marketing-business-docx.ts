/**
 * Seed Marketing & Business courses from `data/CARSI_Marketing_Business_6_Courses.docx`.
 * Lesson bodies are stored as plain text (no HTML tags).
 *
 *   DATABASE_URL="..." npx tsx scripts/seed-marketing-business-docx.ts
 *   DATABASE_URL="..." npx tsx scripts/seed-marketing-business-docx.ts --replace
 *   npx tsx scripts/seed-marketing-business-docx.ts --dry-run
 */
...
import { parseMarketingBusinessCompendium } from '@/lib/seed/marketing-business-docx';
import { prisma } from '@/lib/prisma';
import { DEFAULT_INSTRUCTOR_ID, ensureCatalogInstructor } from '@/lib/server/course-catalog-sync';

const __dirname = dirname(fileURLToPath(import.meta.url));
const DOCX_PATH = join(__dirname, '..', 'data', 'CARSI_Marketing_Business_6_Courses.docx');
...
async function main() {
  ...
  const courses = parseMarketingBusinessCompendium(paras);
