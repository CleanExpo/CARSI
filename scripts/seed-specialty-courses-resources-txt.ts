/**
 * Seed from `data/specialty_courses_resources.txt` (glass/duct courses, resources, memberships).
 * Lesson bodies are plain text (no HTML).
 *
 *   DATABASE_URL="..." npx tsx scripts/seed-specialty-courses-resources-txt.ts
 *   DATABASE_URL="..." npx tsx scripts/seed-specialty-courses-resources-txt.ts --replace
 *   npx tsx scripts/seed-specialty-courses-resources-txt.ts --dry-run
 */
...
import { parseSpecialtyCoursesResourcesTxt } from '@/lib/seed/specialty-courses-resources-txt';
import { prisma } from '@/lib/prisma';
import { DEFAULT_INSTRUCTOR_ID, ensureCatalogInstructor } from '@/lib/server/course-catalog-sync';

const __dirname = dirname(fileURLToPath(import.meta.url));
const TXT_PATH = join(__dirname, '..', 'data', 'specialty_courses_resources.txt');
...
async function main() {
  ...
  const courses = parseSpecialtyCoursesResourcesTxt(raw);

  console.log(`Parsed specialty_courses_resources.txt → ${courses.length} items.`);

  for (const c of courses) {
    const chars =
      c.overviewParagraphs.join('').length +
      c.modules.reduce((n, m) => n + m.bodyText.length, 0);
    console.log(`  — [${c.kind}] ${c.title} (${c.slug})  modules=${c.modules.length}  free=${c.isFree}  chars=${chars}`);
  }
      ...
      async (tx) => {
        ...
        await tx.lmsCourse.create({
          data: {
            ...
            isFree: c.isFree,
            category: 'Specialty Courses & Resources',
            isPublished: true,
            modules: {
