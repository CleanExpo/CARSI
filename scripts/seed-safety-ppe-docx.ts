/**
 * Seed Safety & PPE courses from `data/safety_ppe_courses.docx`.
 * Lesson bodies are stored as plain text (no HTML tags).
 *
 *   DATABASE_URL="..." npx tsx scripts/seed-safety-ppe-docx.ts
 *   DATABASE_URL="..." npx tsx scripts/seed-safety-ppe-docx.ts --replace
 *   npx tsx scripts/seed-safety-ppe-docx.ts --dry-run
 */
...
import { parseSafetyPpeCompendium } from '@/lib/seed/safety-ppe-docx';
import { prisma } from '@/lib/prisma';
import { DEFAULT_INSTRUCTOR_ID, ensureCatalogInstructor } from '@/lib/server/course-catalog-sync';

const __dirname = dirname(fileURLToPath(import.meta.url));
const DOCX_PATH = join(__dirname, '..', 'data', 'safety_ppe_courses.docx');
...
async function main() {
  ...
  const courses = parseSafetyPpeCompendium(paras);
