/**
 * Seed from `data/technology_inspection_tools_courses.txt`.
 * Lesson bodies are plain text (no HTML).
 *
 *   DATABASE_URL="..." npx tsx scripts/seed-technology-inspection-tools-txt.ts
 *   DATABASE_URL="..." npx tsx scripts/seed-technology-inspection-tools-txt.ts --replace
 *   npx tsx scripts/seed-technology-inspection-tools-txt.ts --dry-run
 */
...
import { parseTechnologyInspectionToolsTxt } from '@/lib/seed/technology-inspection-tools-txt';
import { prisma } from '@/lib/prisma';
import { DEFAULT_INSTRUCTOR_ID, ensureCatalogInstructor } from '@/lib/server/course-catalog-sync';

const __dirname = dirname(fileURLToPath(import.meta.url));
const TXT_PATH = join(__dirname, '..', 'data', 'technology_inspection_tools_courses.txt');
...
async function main() {
  ...
  const courses = parseTechnologyInspectionToolsTxt(raw);

  console.log(`Parsed technology_inspection_tools_courses.txt → ${courses.length} courses.`);
      ...
      async (tx) => {
        ...
        await tx.lmsCourse.create({
          data: {
            ...
            isFree: c.isFree,
            category: 'Technology & Inspection Tools',
            isPublished: true,
            modules: {
