# Industry Association Partnerships — CARSI LMS

## GP-148 | Priority: P1 High

## Objective

Establish CARSI as a recognised training partner across 7+ Australian industry associations. Convert association endorsements into group licensing revenue and a steady pipeline of member enrolments.

**Targets:**

- 3 signed MOUs within 6 months
- 5 active partnership agreements within 12 months
- 200+ enrolments sourced via association referral codes per year

---

## Target Associations

### 1. BSCAA — Building Services Contractors Association of Australia

| Detail                      | Information                                                                                                                       |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **Relevance**               | Peak body for cleaning, maintenance and facilities contractors. Direct alignment with IICRC cleaning and restoration disciplines. |
| **Proposition**             | Endorsed CPD provider for BSCAA members. Co-branded certificates for IICRC CEC courses.                                               |
| **Contact Strategy**        | Email CEO/National Director → follow up with partnership proposal PDF → video call demo of platform.                              |
| **Partnership Tier Target** | Preferred Training Partner (member discount + newsletter feature)                                                                 |
| **URL**                     | bscaa.com                                                                                                                         |

### 2. ISSA — Worldwide Cleaning Industry Association (ANZ Chapter)

| Detail                      | Information                                                                                                                                  |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **Relevance**               | Global cleaning industry body with strong ANZ presence. Members include commercial cleaning operators, product suppliers, facility managers. |
| **Proposition**             | Co-branded IICRC CEC course bundle exclusive to ISSA ANZ members. Joint webinar series on mould remediation best practice.                       |
| **Contact Strategy**        | Attend ISSA Cleaning & Hygiene Expo → introduce CARSI at the education booth → follow up with co-branding proposal.                          |
| **Partnership Tier Target** | Preferred Training Partner (co-branded bundle + member discount)                                                                             |
| **URL**                     | issa.com (ANZ chapter via ISSA APAC)                                                                                                         |

### 3. LASA — Leading Age Services Australia

| Detail                      | Information                                                                                                                   |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **Relevance**               | Peak body for aged care providers. Mould and indoor air quality in aged care facilities is a growing compliance focus.        |
| **Proposition**             | Member training benefit — discounted IICRC CEC courses relevant to facility maintenance and infection control compliance.         |
| **Contact Strategy**        | Email Membership Director → propose pilot with 3 member organisations → present results to LASA board for formal endorsement. |
| **Partnership Tier Target** | Endorsed Provider (free listing + co-branded certificate) → upgrade to Preferred after pilot                                  |
| **URL**                     | lasa.asn.au                                                                                                                   |

### 4. ACECQA — Australian Children's Education & Care Quality Authority

| Detail                      | Information                                                                                                                                                                |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Relevance**               | Administers the National Quality Framework (NQF) for early childhood and child care. Indoor environmental quality is assessed under Quality Area 3 (Physical Environment). |
| **Proposition**             | NQF-aligned training listing — CARSI courses in mould assessment and indoor air quality mapped to Quality Area 3 requirements.                                             |
| **Contact Strategy**        | Submit training provider application via ACECQA website → follow up with NQF alignment mapping document → seek listing in ACECQA's professional development directory.     |
| **Partnership Tier Target** | Endorsed Provider (directory listing + NQF alignment badge on certificates)                                                                                                |
| **URL**                     | acecqa.gov.au                                                                                                                                                              |

### 5. Master Builders Australia (MBA)

| Detail                      | Information                                                                                                                            |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| **Relevance**               | Peak body for the building and construction industry. Water damage, structural drying and mould during construction are common issues. |
| **Proposition**             | Construction safety training partner — IICRC CEC courses in WRT, ASD and mould assessment as CPD for builder registration renewal.         |
| **Contact Strategy**        | Approach state branches first (MBA VIC, MBA QLD, MBA WA) → local partnership → escalate to national body.                              |
| **Partnership Tier Target** | Preferred Training Partner (CPD recognition + member discount)                                                                         |
| **URL**                     | masterbuilders.com.au                                                                                                                  |

### 6. REIQ / REI National Bodies — Real Estate Institutes

| Detail                      | Information                                                                                                                                                           |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Relevance**               | Property managers handle mould complaints, water damage and building maintenance. IICRC knowledge improves response quality and reduces liability.                    |
| **Proposition**             | CPD-eligible training for property managers — mould assessment, water damage triage and remediation oversight.                                                        |
| **Contact Strategy**        | Start with REIQ (Queensland) — largest state body. Email Professional Development Manager → propose 2-hour CPD webinar as trial → convert to ongoing CPD partnership. |
| **Partnership Tier Target** | Endorsed Provider (CPD listing) → Preferred (annual CPD calendar inclusion)                                                                                           |
| **State Bodies**            | REIQ (QLD), REIV (VIC), REINSW (NSW), REIWA (WA), REISA (SA), REIT (TAS)                                                                                              |

### 7. LGAQ / IPWEA — Local Government & Public Works

| Detail                      | Information                                                                                                                                                                                                                                           |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Relevance**               | Local government facility managers oversee community buildings, libraries, pools and public housing — all susceptible to mould and water damage. IPWEA (Institute of Public Works Engineering Australasia) members manage infrastructure maintenance. |
| **Proposition**             | Government facility manager training — IICRC CEC courses as CPD for council maintenance teams and public works engineers.                                                                                                                                 |
| **Contact Strategy**        | Email LGAQ (QLD) and IPWEA national → propose group licensing trial for 2–3 councils → publish case study → expand nationally.                                                                                                                        |
| **Partnership Tier Target** | Endorsed Provider → Preferred (bulk licensing for council teams)                                                                                                                                                                                      |
| **URLs**                    | lgaq.asn.au, ipwea.org                                                                                                                                                                                                                                |

---

## Partnership Tiers

### Tier 1: Endorsed Provider (Entry Level)

| Benefit                 | Detail                                                                                                     |
| ----------------------- | ---------------------------------------------------------------------------------------------------------- |
| **For the Association** | Free listing in training directory, co-branded completion certificates for members, no cost to association |
| **For CARSI**           | Logo on CARSI website ("Endorsed by..."), access to member communications, credibility signal              |
| **Member Benefit**      | 10% discount on all CARSI courses via unique member code                                                   |
| **Revenue Share**       | None                                                                                                       |
| **Commitment**          | 12-month trial, renewable annually                                                                         |
| **Cost to Association** | $0                                                                                                         |

### Tier 2: Preferred Training Partner (Mid Tier)

| Benefit                 | Detail                                                                                                                   |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **For the Association** | Newsletter feature (quarterly), member discount code, co-branded landing page on carsi.com.au, quarterly training report |
| **For CARSI**           | Featured in member newsletter, speaking slot at annual conference/event, warm leads from discount code                   |
| **Member Benefit**      | 15% discount on all CARSI courses, priority access to new courses, free lead magnet PDF                                  |
| **Revenue Share**       | None (discount funded by CARSI marketing budget)                                                                         |
| **Commitment**          | 12-month agreement, auto-renew                                                                                           |
| **Cost to Association** | $0                                                                                                                       |

### Tier 3: Exclusive Training Partner (Top Tier)

| Benefit                 | Detail                                                                                                                                                               |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **For the Association** | 10% revenue share on all member enrolments, dedicated CARSI training page branded for association, annual training needs analysis report, co-developed course option |
| **For CARSI**           | Mandatory CPD recommendation for members, exclusive provider status (no competing training providers listed), direct integration with member database                |
| **Member Benefit**      | 20% discount, dedicated support contact, priority certificate processing                                                                                             |
| **Revenue Share**       | 10% of net revenue from association-referred enrolments                                                                                                              |
| **Commitment**          | 24-month exclusive agreement                                                                                                                                         |
| **Cost to Association** | $0 (revenue share is net positive for them)                                                                                                                          |

---

## Email Templates — Initial Outreach

### Version 1: Short (< 150 words)

**Subject:** IICRC Training Partnership — [Association Name] Members

---

Dear [Name],

I'm Phill from CARSI (carsi.com.au), an Australian-owned online training platform delivering IICRC CEC-approved courses in restoration, mould remediation and water damage management.

We'd like to explore a training partnership with [Association Name] — offering your members discounted access to 40+ IICRC CEC Accredited courses, all available online and self-paced.

There's no cost to the association. Members receive a discount code and verifiable digital certificates on completion.

Would you be open to a 15-minute call to discuss?

Kind regards,
Phill
CARSI — Restoration Courses & Training Online
carsi.com.au | training@carsi.com.au

---

### Version 2: Medium (~ 250 words)

**Subject:** Training Partnership Opportunity — IICRC CEC Courses for [Association Name] Members

---

Dear [Name],

I'm writing to introduce CARSI (carsi.com.au), an Australian-owned platform delivering IICRC CEC-approved online training in water damage restoration, mould remediation, structural drying and related disciplines.

We believe there's a strong alignment between CARSI's course catalogue and the professional development needs of [Association Name] members. Specifically:

- **40+ IICRC CEC Accredited courses** — covering WRT, ASD, AMRT, CCT and more
- **100% online, self-paced** — no classroom attendance or roster disruption
- **Verifiable digital certificates** — public credential URL for employer and client verification
- **Continuing Education Credits** — recognised IICRC CECs for credential maintenance

We'd like to propose an **Endorsed Provider** partnership at no cost to [Association Name]. Your members would receive:

- A unique discount code for 10–15% off all CARSI courses
- Co-branded completion certificates featuring [Association Name] and IICRC logos
- Access to our downloadable "Australian Tradesperson's Guide to IICRC Certification"

This arrangement has worked well with other industry bodies and we'd welcome the opportunity to discuss how it could benefit [Association Name] members.

Would you be available for a brief call next week?

Kind regards,
Phill
CARSI — Restoration Courses & Training Online
carsi.com.au | training@carsi.com.au

---

### Version 3: Long (~ 400 words)

**Subject:** Proposal: CARSI as Endorsed Training Provider for [Association Name]

---

Dear [Name],

I'm Phill from CARSI (carsi.com.au) — an Australian-owned online training platform specialising in IICRC CEC-approved courses for restoration, cleaning and building maintenance professionals.

I'm reaching out because I believe [Association Name] members could significantly benefit from access to our training catalogue, and I'd like to explore a formal partnership.

**About CARSI**

CARSI delivers 40+ online courses approved by the IICRC (International Institute of Inspection, Cleaning and Restoration Certification). Our platform is purpose-built for working tradespeople and professionals — courses are 100% online, self-paced, and can be completed on any device. Each course earns verified Continuing Education Credits (CECs) and a digital certificate with a public verification URL.

**Why This Matters for [Association Name] Members**

[Customise this paragraph per association — examples below]

- _For BSCAA:_ Cleaning contractors increasingly need IICRC credentials to win commercial tenders and insurance-related work. Our courses help your members compete.
- _For LASA:_ Mould in aged care facilities is a growing compliance concern. Our training equips facility maintenance teams with recognised credentials.
- _For Master Builders:_ Water damage during construction causes costly defects. Our WRT and ASD courses train site teams to respond correctly.

**Partnership Proposal**

We propose starting as an **Endorsed Provider** — free of charge to [Association Name]:

1. CARSI listed in your training/CPD directory
2. Unique member discount code (10–15% off all courses)
3. Co-branded certificates for member completions
4. Quarterly training uptake report for your records

If the partnership delivers value, we're open to upgrading to **Preferred Training Partner** status — which includes newsletter features, conference speaking, and enhanced member benefits.

**No cost. No risk. Immediate member value.**

I've attached a one-page partnership overview. Would you be open to a 15-minute introductory call? I'm happy to work around your availability.

Kind regards,
Phill
CARSI — Restoration Courses & Training Online
carsi.com.au | training@carsi.com.au

Attachment: CARSI-Partnership-Overview.pdf

---

## Partnership Proposal — One-Page Value Proposition

### For Association Board/Executive Presentation

---

**CARSI — Training Partnership for [Association Name]**

**Who We Are**
CARSI (carsi.com.au) is an Australian-owned online training platform delivering 40+ IICRC CEC-approved courses in restoration, mould remediation, water damage management and related disciplines.

**What We Offer Your Members**
| Benefit | Detail |
|---------|--------|
| Discounted Training | 10–20% off all courses via unique member code |
| Online, Self-Paced | No classroom, no roster disruption, any device |
| Verifiable Credentials | Digital certificates with public verification URL |
| IICRC CECs | Recognised Continuing Education Credits |
| Group Licensing | Bulk pricing for organisations enrolling teams |

**What We Offer the Association**
| Benefit | Detail |
|---------|--------|
| New Member Benefit | Tangible training discount your members actually use |
| Co-Branded Certificates | Association logo on completion certificates |
| Quarterly Reports | Training uptake data for your annual report |
| Revenue Share (Tier 3) | 10% of net revenue from member enrolments |
| Zero Cost | No fees, no infrastructure, no admin overhead |

**Credibility**

- IICRC CEC-approved courses (board-voted)
- 40+ courses across 5 IICRC disciplines
- Built for Australian tradespeople and professionals
- Digital-first: verifiable credentials, LinkedIn sharing, employer audit trail

**Next Step**
15-minute introductory call with Phill → pilot with 3 member organisations → formal endorsement.

Contact: training@carsi.com.au | carsi.com.au

---

## Group Licensing Pricing

| Seats | Annual Price (AUD) | Per-Seat Cost | Discount vs Individual        |
| ----- | ------------------ | ------------- | ----------------------------- |
| 10    | $1,750             | $175          | 78% off subscription          |
| 25    | $3,900             | $156          | 80% off subscription          |
| 50    | $6,900             | $138          | 83% off subscription          |
| 100+  | Custom             | Negotiated    | Contact training@carsi.com.au |

**What's included per seat:**

- Unlimited access to all 40+ IICRC CEC Accredited courses for 12 months
- All CECs earned and tracked per user
- Digital certificate per course completion
- Admin dashboard: enrolment tracking, completion rates, CEC reports
- Dedicated account manager (50+ seats)

**Association-specific pricing:** Associations at Tier 2 or Tier 3 receive an additional 5% off group licensing for their members' organisations.

---

## Success Metrics

### Per Partnership

| Metric                              | Target (6 Months) | Target (12 Months) |
| ----------------------------------- | ----------------- | ------------------ |
| MOU Signed                          | Yes               | Renewed            |
| Member Discount Code Activations    | 50                | 150                |
| Enrolments via Association Referral | 30                | 100                |
| Revenue from Partnership            | $4,425 AUD        | $14,750 AUD        |
| Net Promoter Score (partner survey) | 7+                | 8+                 |

### Aggregate (All Partnerships)

| Metric                              | Target (6 Months) | Target (12 Months) |
| ----------------------------------- | ----------------- | ------------------ |
| Signed MOUs                         | 3                 | 5                  |
| Total Member Code Activations       | 150               | 500                |
| Total Referral Enrolments           | 80                | 250                |
| Total Revenue (association channel) | $11,800 AUD       | $36,875 AUD        |
| Active Tier 2+ Partnerships         | 1                 | 3                  |

### Reporting Cadence

- **Monthly:** Discount code activations, enrolments per association, revenue per partner
- **Quarterly:** Partnership health review — engagement level, upgrade potential, churn risk
- **Annually:** Full partnership channel report — ROI, renewal decisions, new target associations

---

## Outreach Timeline

| Week  | Action                                                                                  |
| ----- | --------------------------------------------------------------------------------------- |
| 1     | Finalise partnership proposal PDF and one-pager                                         |
| 2     | Send initial outreach emails to BSCAA, ISSA, LASA (Tier 1 targets)                      |
| 3     | Follow up with phone calls to non-responders                                            |
| 4     | Send outreach to ACECQA, Master Builders, REIQ (Tier 2 targets)                         |
| 5–6   | Schedule and conduct introductory calls                                                 |
| 7–8   | Send formal partnership proposals to interested associations                            |
| 9–10  | Negotiate terms, finalise MOUs                                                          |
| 11–12 | Onboard first partners: create discount codes, co-branded assets, member communications |
| 13+   | Launch pilot → track metrics → iterate                                                  |

---

## Launch Checklist

- [ ] Partnership proposal PDF designed and approved
- [ ] One-page value proposition formatted for board presentation
- [ ] Email templates customised per association
- [ ] Discount code system configured in CARSI platform (unique code per association)
- [ ] Co-branded certificate template created (association logo slot)
- [ ] Group licensing pricing page live on carsi.com.au
- [ ] CRM/spreadsheet for tracking outreach status per association
- [ ] Admin dashboard: filter enrolments by referral source/discount code
- [ ] Quarterly reporting template created
- [ ] training@carsi.com.au inbox monitored and responsive
