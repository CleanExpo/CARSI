# Google Ads Campaign — CARSI LMS

## GP-146 | Priority: P1 High

## Objective

Drive qualified enrolments across 6 industry verticals via Google Search and Display campaigns. Target: 65–110 enrolments/month at $45–65 AUD CPA within 90 days of launch.

---

## Campaign Structure

| #   | Campaign Name               | Vertical                    | Daily Budget | Monthly Budget |
| --- | --------------------------- | --------------------------- | ------------ | -------------- |
| 1   | CARSI — Restoration         | Restoration / Water Damage  | $50          | $1,500         |
| 2   | CARSI — Mining              | Mining & Resources          | $35          | $1,050         |
| 3   | CARSI — Aged Care           | Aged Care & Healthcare      | $25          | $750           |
| 4   | CARSI — Healthcare          | Hospitals & Health Services | $15          | $450           |
| 5   | CARSI — Commercial Cleaning | Commercial & Facilities     | $15          | $450           |
| 6   | CARSI — Construction        | Construction & Building     | $10          | $300           |
|     | **Total**                   |                             | **$150**     | **$4,500**     |

Adjust allocation monthly based on vertical ROAS. Start conservative, scale winners.

---

## Keyword Groups by Vertical

### 1. Restoration

**Exact Match**

- [IICRC certification Australia]
- [water damage restoration course]
- [mould remediation training online]
- [IICRC WRT certification]
- [flood restoration training Australia]
- [structural drying course online]
- [IICRC CEC courses]
- [fire and smoke restoration course]

**Broad Match Modifier**

- +IICRC +training +Australia
- +water +damage +restoration +course
- +mould +remediation +certification
- +restoration +technician +training +online

### 2. Mining

**Exact Match**

- [mining safety training online]
- [FIFO training course]
- [WHS mould compliance training]
- [mould assessment training mining]
- [FIFO accommodation mould course]
- [mining water ingress training]
- [site supervisor safety training online]

**Broad Match Modifier**

- +mining +safety +training +online
- +FIFO +mould +training
- +WHS +compliance +course +mining
- +mould +assessment +mining +site

### 3. Aged Care

**Exact Match**

- [infection control training aged care]
- [IICRC aged care certificate]
- [mould training aged care facility]
- [aged care hygiene compliance course]
- [mould remediation aged care]
- [indoor air quality training aged care]

**Broad Match Modifier**

- +infection +control +training +aged +care
- +mould +training +facility +management
- +aged +care +compliance +course
- +hygiene +certification +aged +care

### 4. Healthcare

**Exact Match**

- [hospital mould remediation training]
- [healthcare infection control course online]
- [IICRC certification healthcare]
- [indoor air quality training hospital]
- [water damage response training healthcare]

**Broad Match Modifier**

- +healthcare +mould +training
- +hospital +remediation +course
- +infection +control +certification +online

### 5. Commercial Cleaning

**Exact Match**

- [commercial cleaning certification Australia]
- [IICRC cleaning technician course]
- [carpet cleaning certification online]
- [commercial mould remediation training]
- [cleaning business certification Australia]
- [IICRC CCT certification]

**Broad Match Modifier**

- +commercial +cleaning +certification +Australia
- +IICRC +cleaning +course
- +mould +remediation +training +cleaning +business

### 6. Construction

**Exact Match**

- [construction mould training Australia]
- [water damage prevention course construction]
- [building restoration training online]
- [structural drying certification construction]
- [construction safety compliance training]

**Broad Match Modifier**

- +construction +mould +training
- +building +restoration +certification
- +water +damage +course +construction

---

## Negative Keywords (Account-Level)

Apply across all campaigns to prevent wasted spend:

- free
- university
- TAFE
- accredited qualification
- RPL (recognition of prior learning)
- white card
- government funded
- apprenticeship
- traineeship
- degree
- diploma
- bachelor
- master
- PhD
- download
- template
- sample
- DIY
- YouTube
- Reddit
- salary
- jobs
- hiring
- vacancy
- how much do
- cheap

---

## Ad Copy — Responsive Search Ads (RSA)

### Campaign 1: Restoration

**Variation A — Authority**
| Element | Copy |
|---------|------|
| Headline 1 | IICRC CEC Accredited Restoration Courses |
| Headline 2 | Online Training — Study at Your Pace |
| Headline 3 | Earn CECs From Anywhere in Australia |
| Headline 4 | WRT, ASD, AMRT Certifications |
| Headline 5 | Trusted by 500+ Australian Technicians |
| Description 1 | Earn IICRC CECs toward your certification in water damage restoration, mould remediation and structural drying. 100% online, self-paced. Start today. |
| Description 2 | Earn Continuing Education Credits recognised across Australia. Complete courses between jobs — no roster disruption. Verifiable digital certificates included. |

**Variation B — Urgency**
| Element | Copy |
|---------|------|
| Headline 1 | IICRC CECs — Renew Your Credentials |
| Headline 2 | Don't Let Your Certification Lapse |
| Headline 3 | Online Courses — Start in 5 Minutes |
| Headline 4 | Recognised Across Australia |
| Headline 5 | Digital Certificate on Completion |
| Description 1 | Your IICRC credentials require ongoing CECs. Complete approved courses online from CARSI — Australia's specialist restoration training platform. Enrol now. |
| Description 2 | Self-paced IICRC CEC courses designed for working technicians. Earn CECs, download your certificate, and share it on LinkedIn. No classroom attendance required. |

**Variation C — Value**
| Element | Copy |
|---------|------|
| Headline 1 | Restoration Training Bundles from $295 |
| Headline 2 | IICRC CEC Accredited — Earn Real CECs |
| Headline 3 | 40+ Courses Available Online |
| Headline 4 | Subscription Access — $795/Year |
| Headline 5 | Upskill Without Leaving the Job Site |
| Description 1 | Access 40+ IICRC CEC-approved courses with a CARSI subscription. Water damage, mould, fire restoration and more. $795 AUD/year — unlimited access. |
| Description 2 | Save with course bundles or go all-in with annual subscription. Every course earns verified CECs. Public credential URL included for contractor pre-qualification. |

### Campaign 2: Mining

**Variation A — Compliance**
| Element | Copy |
|---------|------|
| Headline 1 | Mining Safety Training — 100% Online |
| Headline 2 | WHS Mould Compliance Courses |
| Headline 3 | IICRC Certifications for Mining Sites |
| Headline 4 | FIFO-Friendly — Study Between Shifts |
| Headline 5 | Reduce Mould Liability on Site |
| Description 1 | Mould in FIFO accommodation is a costly liability. Train your team in IICRC CEC Accredited mould assessment and remediation — fully online, no roster disruption. |
| Description 2 | WHS compliance training designed for mining professionals. Earn verifiable IICRC credentials your HSE team can audit. Digital certificates with public verification URL. |

**Variation B — ROI**
| Element | Copy |
|---------|------|
| Headline 1 | FIFO Mould Training — Protect Your Site |
| Headline 2 | Avoid $500K+ Mould Liability Costs |
| Headline 3 | Online IICRC CEC Courses for Mining Teams |
| Headline 4 | Group Licensing Available |
| Headline 5 | Trusted by WA & QLD Mining Sites |
| Description 1 | One mould incident at a FIFO camp can cost $500K+. Train supervisors in IICRC CEC Accredited mould assessment for a fraction of the cost. Group rates available. |
| Description 2 | Upskill your mining crew in water damage, mould and structural drying. Self-paced courses designed for shift workers. Bulk enrolment discounts for 10+ seats. |

**Variation C — Speed**
| Element | Copy |
|---------|------|
| Headline 1 | Start Mining Safety Training Today |
| Headline 2 | IICRC CEC Courses — Instant Online Access |
| Headline 3 | No Classroom — No Roster Disruption |
| Headline 4 | Certificate on Completion |
| Headline 5 | WHS Mould & Water Damage Courses |
| Description 1 | Enrol your team in minutes. IICRC CEC Accredited courses in mould assessment, water damage response and structural drying. Ideal for FIFO rosters — 100% online. |
| Description 2 | Mining safety training that fits around shift work. Complete modules on any device, earn your certificate, and verify credentials via public URL. Start now. |

### Campaign 3: Aged Care

**Variation A — Compliance**
| Element | Copy |
|---------|------|
| Headline 1 | Aged Care Mould Training — Online |
| Headline 2 | Infection Control Certification |
| Headline 3 | IICRC CEC Accredited for Facility Managers |
| Headline 4 | Protect Residents & Staff |
| Headline 5 | Comply with Aged Care Standards |
| Description 1 | Mould and water damage in aged care facilities put vulnerable residents at risk. Train your team in IICRC CEC Accredited remediation — fully online, self-paced. |
| Description 2 | Meet facility compliance requirements with certified mould and infection control training. Digital certificates included. Group licensing for multi-site operators. |

**Variation B — Risk**
| Element | Copy |
|---------|------|
| Headline 1 | Mould in Aged Care — Are You Compliant? |
| Headline 2 | IICRC Training for Facility Teams |
| Headline 3 | Online Courses — No Staff Downtime |
| Headline 4 | Verifiable Digital Certificates |
| Headline 5 | Group Rates for Multi-Site Operators |
| Description 1 | Aged care quality standards require proactive mould management. Ensure your team holds IICRC-recognised credentials. Enrol online — no disruption to care rosters. |
| Description 2 | From facility managers to maintenance staff — everyone handling mould should be trained. CARSI offers IICRC CEC Accredited courses tailored to aged care environments. |

**Variation C — Value**
| Element | Copy |
|---------|------|
| Headline 1 | Aged Care Staff Training from $79/Course |
| Headline 2 | IICRC Mould & Remediation Courses |
| Headline 3 | Unlimited Access — $795/Year |
| Headline 4 | Train Your Whole Team Online |
| Headline 5 | Australian-Owned Training Platform |
| Description 1 | Affordable IICRC CEC Accredited training for aged care teams. Individual courses from $79 or unlimited annual access at $795. Bulk discounts for 10+ staff. |
| Description 2 | Australian-owned, IICRC CEC Accredited training designed for aged care. Mould assessment, remediation, indoor air quality. Start your team today — instant access. |

### Campaign 4: Healthcare

**Variation A**
| Element | Copy |
|---------|------|
| Headline 1 | Hospital Mould Training — IICRC CEC Accredited |
| Headline 2 | Healthcare Remediation Courses Online |
| Headline 3 | Protect Patients & Meet Compliance |
| Headline 4 | Digital Certificates Included |
| Headline 5 | Self-Paced — No Roster Disruption |
| Description 1 | Healthcare facilities face strict indoor air quality standards. Train your maintenance team in IICRC CEC Accredited mould remediation and water damage response. 100% online. |
| Description 2 | From hospitals to medical centres — mould is a patient safety issue. Earn IICRC credentials your compliance team can verify. Group enrolment rates available. |

### Campaign 5: Commercial Cleaning

**Variation A**
| Element | Copy |
|---------|------|
| Headline 1 | IICRC Cleaning Certification — Online |
| Headline 2 | Upskill Your Cleaning Business |
| Headline 3 | CCT, WRT & Mould Courses Available |
| Headline 4 | Win More Contracts with IICRC Creds |
| Headline 5 | Australian-Owned Training Platform |
| Description 1 | Stand out from competitors with IICRC certifications. Carpet cleaning, mould remediation, water damage — all available online. Earn CECs and grow your business. |
| Description 2 | Cleaning franchise owners and operators — add IICRC credentials to your business. Online courses, verifiable certificates, and CECs that prove your expertise. |

### Campaign 6: Construction

**Variation A**
| Element | Copy |
|---------|------|
| Headline 1 | Construction Mould Training — Online |
| Headline 2 | IICRC Water Damage Courses |
| Headline 3 | Structural Drying Certification |
| Headline 4 | Protect Your Builds from Mould |
| Headline 5 | Digital Certificate on Completion |
| Description 1 | Water ingress and mould during construction cause costly delays. Train your site team in IICRC CEC Accredited structural drying and mould prevention. Fully online. |
| Description 2 | Builders and project managers — reduce defect liability with IICRC credentials. Self-paced courses in water damage, mould assessment and restoration. Start today. |

---

## Bidding Strategy

| Parameter                  | Setting                                                     |
| -------------------------- | ----------------------------------------------------------- |
| Strategy                   | Target CPA (tCPA)                                           |
| Target CPA                 | $55 AUD (midpoint of $45–65 range)                          |
| Initial phase (weeks 1–4)  | Maximise Conversions (let Google learn)                     |
| Transition to tCPA         | After 30+ conversions accumulated                           |
| Bid adjustments — Device   | +15% mobile (tradies search on phone)                       |
| Bid adjustments — Location | +20% WA, QLD (mining); +10% NSW, VIC (restoration/cleaning) |
| Bid adjustments — Schedule | +10% Mon–Fri 6am–9am, 7pm–11pm (before/after shift)         |

---

## Budget Recommendation

| Phase                    | Monthly Budget | Focus                                                         |
| ------------------------ | -------------- | ------------------------------------------------------------- |
| Month 1–3 (Learning)     | $3,000 AUD     | All 6 campaigns, even split, gather data                      |
| Month 4–6 (Optimisation) | $4,000 AUD     | Shift budget to top 3 performing verticals                    |
| Month 7–12 (Scale)       | $5,000+ AUD    | Double down on ROAS-positive verticals, pause underperformers |

**Break-even calculation:** At $55 CPA and $295 average course value, ROAS = 5.4x. At $795 subscription, ROAS = 14.5x.

---

## Landing Page Mapping

| Campaign            | Landing Page                     | Notes                                 |
| ------------------- | -------------------------------- | ------------------------------------- |
| Restoration         | /courses (filtered: restoration) | Primary catalogue with IICRC badge    |
| Mining              | /industries/mining               | Dedicated vertical page (from GP-136) |
| Aged Care           | /industries/aged-care            | Create dedicated landing page         |
| Healthcare          | /industries/healthcare           | Create dedicated landing page         |
| Commercial Cleaning | /industries/commercial-cleaning  | Create dedicated landing page         |
| Construction        | /industries/construction         | Create dedicated landing page         |

Each landing page must include:

- IICRC logo and "CEC-approved" badge
- Relevant course cards for that vertical
- Social proof (enrolment count, testimonials)
- Clear CTA: "Enrol Now" or "Start Free Trial"
- FAQ section addressing vertical-specific objections

---

## Conversion Tracking

### Primary Conversions (bid-optimised)

| Event                 | Trigger                                      | Value               |
| --------------------- | -------------------------------------------- | ------------------- |
| Enrolment Complete    | `/api/lms/enrollments` POST success          | Dynamic ($79–$295)  |
| Subscription Purchase | Stripe checkout success → /subscribe/success | $795                |
| Bundle Purchase       | Cart checkout complete                       | Dynamic ($195–$495) |

### Secondary Conversions (observation only)

| Event                   | Trigger                              | Value           |
| ----------------------- | ------------------------------------ | --------------- |
| Email Signup            | Newsletter form submit               | $5 (estimated)  |
| Lead Magnet Download    | PDF download click                   | $10 (estimated) |
| Course Page View (>30s) | Time on page > 30 seconds            | $1 (estimated)  |
| Free Lesson Start       | First lesson accessed (non-enrolled) | $15 (estimated) |

### Implementation

1. Google Tag Manager container on all pages
2. GA4 enhanced e-commerce events for enrolment funnel
3. Google Ads conversion linker tag
4. Offline conversion import for subscription renewals (monthly upload)

---

## Monthly Reporting Metrics

| Metric                   | Target (Month 3) | Target (Month 6) |
| ------------------------ | ---------------- | ---------------- |
| Impressions              | 50,000           | 80,000           |
| Clicks                   | 2,500            | 4,000            |
| CTR                      | 5.0%             | 5.5%             |
| Conversions (enrolments) | 55               | 85               |
| Conversion Rate          | 2.2%             | 2.5%             |
| CPA                      | $55 AUD          | $47 AUD          |
| ROAS                     | 5.0x             | 6.5x             |
| Revenue                  | $16,225          | $25,075          |
| Spend                    | $3,000           | $4,000           |

### Reporting Cadence

- **Weekly**: Spend, clicks, conversions, CPA by campaign (Google Ads dashboard)
- **Fortnightly**: Search term review — add negatives, promote winners to exact match
- **Monthly**: Full performance report with vertical comparison and budget reallocation recommendation
- **Quarterly**: Strategy review — pause/launch verticals, refresh ad copy, update landing pages

---

## Launch Checklist

- [ ] Google Ads account created and verified (ABN billing)
- [ ] Conversion tracking tags deployed via GTM
- [ ] GA4 linked to Google Ads
- [ ] All 6 campaigns built with ad groups and keywords
- [ ] Negative keyword list applied at account level
- [ ] RSA ads approved (allow 24–48 hours for review)
- [ ] Landing pages live for all 6 verticals
- [ ] Call tracking number provisioned (optional — for phone conversions)
- [ ] Budget set to $100/day initially ($3,000/month)
- [ ] Automated rules: pause keywords with CPA > $100 after 200+ impressions
